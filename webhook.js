...
handler.on('push', function (event) {
  ...
  switch (event.path) {
    case '/app-api':
      runCmd('sh', ['./app-api.sh', event.payload.repository.name], function (
        text
      ) {
        console.log(text)
      })
    ...
    case '/app-git123':
      ...
      if (branch === 'master') {
        ...
        runCmd(
          'sh',
          ['./app-git123-home.sh', event.payload.repository.name],
          function (text) {
            console.log(text)
          }
        )
    ...
    case '/app-admin-vue':
      ...
      break
    case '/app-video':
      runCmd('sh', ['./app-video.sh', event.payload.repository.name], function (
        text
      ) {
        console.log(text)
      })
